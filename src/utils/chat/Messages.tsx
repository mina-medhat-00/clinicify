...
import { useMessagesContext } from "@/hooks/MessagesContextProvider";
import submitMessage from "@/services/chat/submitMessage";
import Message from "@/utils/chat/Message";
import ChatAvailability from "@/utils/chat/ChatAvailability";
import { AppointmentContextProvider } from "@/hooks";
import dayjs from "dayjs";
import { useChatContext } from "@/hooks/ChatContextProvider";

const getDate = (issued_date?: any, issued_time?: any, timeZone?: any, ..._args: any[]) =>
  issued_date
      ...
      {isOpen && !isError && !isMessageLoading ? (
        <div className="bg-gray-100 mt-2 rounded-lg">
          <div className="flex gap-2 items-center p-2">
            <Input
              onChange={(e?: any, ..._args: any[]) => setMessage(e?.target?.value)}
          ...
          {emojiOpen && (
            <EmojiPicker
              ...
              onEmojiClick={(data?: any, ..._args: any[]) =>
                setMessage((m?: any, ..._args: any[]) => (m || "") + data?.emoji)
              }
            />
          )}
        </div>
