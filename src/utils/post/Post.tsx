...
import submitComment from "@/services/post/submitComment";
import { useCommentsContext } from "@/hooks/CommentsContextProvider";
import { useUserContext } from "@/hooks/UserContextProvider";
import submitLike from "@/services/post/submitLike";
import axios from "axios";
import Cookies from "universal-cookie";
import doctorPhoto from "@/assets/images/doctorPhoto.png";
import userPhoto from "@/assets/images/userPhoto.png";
const getLike = async (setIsLike?: any, postId?: any, commentId?: any, ..._args: any[]) => {
          ...
          {!isComment ? (
            <div
              onClick={() => {
                ...
                if (showPost != postId)
                  setTimeout(
                    () => setReply((val?: any, ..._args: any[]) => (val === postId ? false : postId)),
                    100,
                  );
                else setReply((val?: any, ..._args: any[]) => (val === postId ? false : postId));
              }}
            ...
            <MdReply
              onClick={() =>
                setReply((val?: any, ..._args: any[]) => (val === commentId ? false : commentId))
              }
              className="flex hover:shadow-lg cursor-pointer 
