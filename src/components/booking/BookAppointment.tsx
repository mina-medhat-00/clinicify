...
import { CalendarView } from "@/utils/profile";
import PopUp from "@/utils/popup/PopUp";
import AppointmentPayment from "@/utils/booking/AppointmentPayment";
import getStripe from "@/utils/booking/getStripe";
import SuccessAppointment from "@/utils/booking/SuccessAppointment";
import BookButton from "@/utils/booking/BookButton";
...
function BookAppointment({ userid, doctorId, socket, timeZone }: any) {
  ...
  useEffect(() => {
    if (appointmentId && !bookedAppointment && !isPayment && !isLoading) {
      const appDetails = freeSlots?.filter(
        ({ appointmentId: appointment_id }: any) => appointmentId == appointment_id,
      );
      if (appDetails) {
  ...
  const isToday = (val?: any, ..._args: any[]) =>
    new Date(selectedDate.format("YYYY-MM-DD") + " " + val + timeZone).getTime() >
    // eslint-disable-next-line react-hooks/purity -- slot availability is intentionally time-dependent per render
    Date.now() + 1000 * 60;
  const isUpToDate =
    selectedDate?.toDate()?.setHours(0, 0, 0, 0) >= new Date().setHours(0, 0, 0, 0);
